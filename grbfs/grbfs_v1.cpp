...
{
	queue<pair<int, int> > temp_queue(in_queue);
...
int main() {
	...
	for (int q = 0; q < queries; q++) {
		...
		/* Create a graph based on adjacency list */
		vector<vector<int> > graph(nodes, vector<int>());

		for (int e = 0; e < edges; e++) {
			cin >> first_node >> second_node;

			/* Insert into adjacency list for both nodes */
			vector<int> &adj_list_1 = graph[first_node - 1];
			adj_list_1.push_back(second_node);

			vector<int> &adj_list_2 = graph[second_node - 1];
			adj_list_2.push_back(first_node);
		}
		...
		bfs_queue.push(pair<int, int>(start_node, start_node));

		while (! bfs_queue.empty()) {
			pair<int, int> queue_elem = bfs_queue.front();
			bfs_queue.pop();
			...
			vector<int> &adj_list = graph[next_node - 1];

			for (int i = 0; i < adj_list.size(); i++) {
				if (distances[adj_list[i] - 1] == 0) { /* Not yet visited */
					bfs_queue.push(pair<int, int>(adj_list[i], next_node));
				}
			}
