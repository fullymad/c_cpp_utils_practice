/*****************************************************************************
<TEMP NOTES>
Build matched_indices up front
Eliminate FIRST_TIME_PROCESSED, matched_indices_staging
Eliminate save_matched_str2_pos_from_staging()
Retain get_next_matched_str2_pos()
</TEMP NOTES>
...
*   integers 6, 4 and 0 as those are the indices of the string where 'd' is
...
*   6, 4 and 0 in this case) as in {3, 6, 4, 0}.
*   Note: The indices are in reverse order to conform to existing logic in
*   caller. There is no other special reason and can be changed if needed.
*
* Input:
...
)
{
	...
	for (source_i = source_len - 1; source_i >= 0; source_i--) {
...
)
{
	...
	for (i = 0; i < s1_len; i++) {
		...
		match_num = 1;
		next_match_pos = get_next_matched_str2_pos(map_to_s2_indices,
				s1[i], match_num);
		...
		while (start_s2_pos >= 0) {
			...
			match_num++;
			start_s2_pos = get_next_matched_str2_pos(map_to_s2_indices,
					s1[i], match_num);
