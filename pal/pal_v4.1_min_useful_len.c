...
*     'parse_direction'
*   min_useful_len: Caller is only interested in palindromes of at least this
*     length. If not, will take 0 as length.
...
)
{
	...
	if (PARSE_FORWARD == parse_direction) {
		...
		while (1) {
			...
			else if (next_match_pos - start_index + 1 < min_useful_len) {
				count = 0;
				break;
			}
		...
		while (1) {
			...
			else if (start_index - next_match_pos + 1 < min_useful_len) {
				count = 0;
				break;
			}
			/* Check if this is a subset of a palindrome already encountered */
	...
	/* Update cache with the computed length (if any) */
	if (count != 0) {
	...
	for (i = 0; i < s1_len; i++) {
		...
		while (start_s2_pos >= 0) {
			...
			if (matched) {
				...
				/* Get palindrome lengths from rest of s1 or s2, specifying
				 * minimum length interested in.
				 */
				min_useful_len = result_len - match_len;
